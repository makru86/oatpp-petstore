...
void ApiResponseDTOTest::onRun() {
    ...
    OATPP_ASSERT(jsonString ==
                 objectMapper->writeToString(object));

    /* Assert that deserialized object as expected */
    OATPP_ASSERT(object ==
                 objectMapper->readFromString<oatpp::Object<ApiResponseDTO>>(
                         jsonString));
}

ApiResponseDTOTest::ApiResponseDTOTest() : UnitTest("TEST[ApiResponseDTO]"),
                             objectMapper(ObjectMapper::createShared()) {}
